...
function fetchPageWithRetry(subject, page, maxRetries = 3) {
  return new Promise((resolve, reject) => {
    const tryFetch = (n) => {
      api.getQuestions(subject, page, SYNC_PAGE_SIZE).then(resolve).catch(err => {
        if (n < maxRetries) {
...
/** 分页拉取全部题目并缓存，onProgress(page, totalPages) 回调 */
function syncAllQuestions(subject, serverVersion, onProgress) {
  return fetchPageWithRetry(subject, 1).then(first => {
    const total = first.total || 0
    const totalPages = Math.ceil(total / SYNC_PAGE_SIZE)
    ...
    return pages.reduce((promise, page) => {
      return promise.then(() => fetchPageWithRetry(subject, page).then(data => {
